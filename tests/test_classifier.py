...
# ===========================================================================
# vector.py unit tests (FR-07 scope, always-run subset per §7.2 ARCH-M3)
# ===========================================================================


def test_vector_import_성공() -> None:
    """AC8/FR-07: pptx_md.vector imports without error."""
    import pptx_md.vector  # noqa: F401


def test_vector_libreoffice_available_bool() -> None:
    """FR-07: libreoffice_available() returns a bool without raising."""
    from pptx_md.vector import libreoffice_available

    result = libreoffice_available()
    assert isinstance(result, bool)


def test_vector_unsupported_ext_returns_none() -> None:
    """FR-07: convert_vector_to_png with unsupported ext returns None."""
    from pptx_md.vector import convert_vector_to_png

    result = convert_vector_to_png(b"fake", "png")
    assert result is None


def test_vector_no_libreoffice_returns_none(monkeypatch: pytest.MonkeyPatch) -> None:
    """FR-07: convert_vector_to_png returns None when LibreOffice absent."""
    import shutil as _shutil
...
def test_vector_no_raise_on_corrupt_data(monkeypatch: pytest.MonkeyPatch) -> None:
    """FR-07: convert_vector_to_png never raises, even on corrupt data."""
    import shutil as _shutil
    ...
    monkeypatch.setattr(_shutil, "which", lambda _name: None)
    try:
        convert_vector_to_png(b"", "emf")
        convert_vector_to_png(b"\x00" * 50, "wmf")
    except Exception as exc:
        pytest.fail(f"convert_vector_to_png raised unexpectedly: {exc}")


def test_vector_ext_constants() -> None:
    """FR-07: VECTOR_EXTS contains emf and wmf."""
    from pptx_md.vector import VECTOR_EXTS
